...
def file_name_maker_met_pad(amount_of_rolls, posix_destination_pad, filename : "str", exp=".csv"):
    ...
    for naam in range(amount_of_rolls):
        manufactored_name_with_path = f'{Path(posix_destination_pad / filename)}_{naam+1:>{0}{5}}{exp}'
...
def rollen_maker(basis_csv_bestand_in, posix_destination_pad, wikkel: int ,filename: "str", exp=".csv"):

    noshort_rolls_met_4_kolommen  = pd.read_csv(Path(basis_csv_bestand_in), ";", dtype="str")
    ...
    def file_name_maker_met_pad():
        ...
        for naam in range(lengte_van_data_frame):
            manufactored_name_with_path = f'{posix_destination_pad / filename}_{naam + 1:>{0}{5}}{exp}'
            # print(manufactored_name_with_path)
            man_fac_name_with_path_lijst.append(manufactored_name_with_path)
    ...
    def noshort_rolls_4kolommen(omschrijving_sluit, sluit_barcode, beeld, aantal_etiketten, wikkel):
        """verwijzing naar plek  basis_csv_bestand-in"""


        inloop = pd.DataFrame([["",0,"","stans.pdf"] for x in range(2)],
                                    columns=['omschrijving_sluit_1', 'sluit_barcode_1', 'aantal_1', 'beeld'], dtype="str")

        sluitetiket = pd.DataFrame(
            [[f"{omschrijving_sluit}", f"{sluit_barcode}", f"{aantal_etiketten}", 'leeg.pdf']],

                                    columns=['omschrijving_sluit_1', 'sluit_barcode_1', 'aantal_1', 'beeld'], dtype="str")

        tussen_rol_en_sluit = pd.DataFrame([["",0,"","stans.pdf"] for x in range(wikkel)],
                                    columns=['omschrijving_sluit_1', 'sluit_barcode_1', 'aantal_1', 'beeld'], dtype="str")

        lichaam_rol = pd.DataFrame([[" ", "100000000000", "", beeld]] * int(aantal_etiketten),
                                   columns=['omschrijving_sluit_1', 'sluit_barcode_1', 'aantal_1', 'beeld'], dtype="str")
    ...
    for i in range(lengte_van_data_frame):
        ...
        llor = noshort_rolls_4kolommen(omschrijving_sluit, sluit_barcode, beeld, aantal_etiketten, wikkel)
        llor.to_csv(csv_naam)

    return csv_namen


inschiet_file = Path(r"C:\Users\Dhr. Ten Hoonte\PycharmProjects\Werk_projekten\Vila_no_short\source\file_in\202011726_inschiet_met_rolnummer.csv")
